...
import com.jrprofessor.platformcommons.ui.gone
import com.jrprofessor.platformcommons.ui.visible
        ...
        fun setData(movie: Movie) {
            binding.apply {
                Glide.with(root.context).load("http://image.tmdb.org/t/p/w185" + movie.posterPath)
                    .into(ivMovieImage)
                Log.d("TAG", "setData: " + "http://image.tmdb.org/t/p/w185" + movie.posterPath)
                tvMovieName.text = movie.title
                if (!movie.releaseDate.isNullOrEmpty()) {
                    tvReleaseDate.text = movie.releaseDate.formatDate()
                    tvReleaseDate.visible()
                } else {
                    tvReleaseDate.gone()
                }
